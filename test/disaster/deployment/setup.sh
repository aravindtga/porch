...
skip_variant_sets=${SKIP_VARIANT_SETS:-false}
skip_variants=${SKIP_VARIANTS:-false}
...
function resetTestWorkload() {
    ...
    package_revisions="$(\
        kubectl_dbcache get packagerevisions -o name | awk -F '/' '{print $2}' | grep -E '(catalog|kpt).*-[12]' \
    ...
    #     echo "Editing $rev"
    #     output="$(porchctl_dbcache rpkg edit "$rev" --workspace "edit-$edited_count")"
    #     edited+="$(awk '{print $1}' <<< "$output")"$'\n'
    #     echo "$output"
    # done
    cloned="$(kubectl_dbcache get packagerevisions | grep -E "clone-" | awk '{print $1}')"
