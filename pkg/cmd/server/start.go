...
func (o *PorchServerOptions) Complete() error {
	...
	if o.CacheType == string(cachetypes.DBCacheType) {
		...
		if !o.UseLegacySync {
			klog.Warningf("DB cache with controller-based sync requires standalone porch-controllers deployment")
		}
	} else if o.CacheType == string(cachetypes.CRCacheType) {
		if o.UseLegacySync {
			klog.Warningf("CR cache with legacy sync - consider using controller-based sync (embedded controller available)")
		}
	}
